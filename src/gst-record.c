...
void * run(void * v) {
	...
	GstElement *pipeline, *v4l2src, *videoconvert, *omxh264enc, *avimux, *filesink;
	GstBus *bus;
	GstMessage *msg;
	...
	/* Create the elements */
	//gst-launch-1.0 -v v4l2src num-buffers=90 ! video/x-raw, width=320, height=240, framerate=30/1 ! videoconvert ! omxh264enc target-bitrate=350000 control-rate=variable ! avimux ! filesink location=file.avi
	...
	if (!pipeline || !v4l2src || !videoconvert || !omxh264enc || !avimux || !filesink) {
	...
	/* Build the pipeline */
	gst_bin_add_many (GST_BIN (pipeline), v4l2src, videoconvert, omxh264enc, avimux, filesink,  NULL);
	fprintf(stderr, "Linking pipeline..\n");
	/* Link the eleemnts */
	if (gst_element_link_filtered(v4l2src , videoconvert, capture_caps) !=TRUE ) {
	  g_printerr ("Could not connect v4l2src to videoconvert.\n");
	...
	if (gst_element_link_filtered(omxh264enc , avimux, h264_caps ) !=TRUE ) {
	  g_printerr ("Could not connect omxh264enc to avimux.\n");
	  gst_object_unref (pipeline);
	  ret_run();
	  status=GST_DIED;
	  sem_post(&gst_off);
	...
	if (gst_element_link(avimux , filesink ) !=TRUE ) {
	  g_printerr ("Could not connect avimux to filesink.\n");
	  gst_object_unref (pipeline);
	  ret_run();
	  status=GST_DIED;
	  sem_post(&gst_off);
	...
	g_object_set( G_OBJECT(omxh264enc), "target-bitrate", target_bitrate, "control-rate", OMX_Video_ControlRateVariable, "interval-intraframes", 2, NULL);
	g_object_set( G_OBJECT(filesink), "location", filename_out, NULL);
...
int main(int argc, char *argv[]) {
	if (argc!=6) {
		fprintf(stdout, "%s Xres Yres target_bitrate[try 500000] seconds fileout\n",argv[0]);
		exit(1);
	}
